...
import { Database, Code2, Terminal, Layers, Users, Lightbulb, GitBranch } from "lucide-react";
function Skills() {
  const languages = technicalSkills.filter((s) => ["C", "Embedded C", "Python", "OOPs", "Data Structures & Algorithms"].includes(s.name));
  const webAndDb = technicalSkills.filter((s) => ["HTML/CSS/JavaScript", "DBMS", "MySQL"].includes(s.name));
  const tools = technicalSkills.filter((s) => ["Linux", "Git & GitHub", "RTOS"].includes(s.name));
  ...
  return <section id="skills" className="py-20 bg-background relative overflow-hidden">
      {
    /* Decorative elements */
  }
      <div className="absolute top-0 right-0 w-1/3 h-1/3 bg-primary/5 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-1/4 h-1/4 bg-blue-600/5 rounded-full blur-[80px] pointer-events-none" />
        ...
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="all" className="mt-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[...languages, ...webAndDb, ...tools, ...designSkills].map((skill, idx) => <SkillCard key={idx} skill={skill} idx={idx} />)}
            </div>
          </TabsContent>

          <TabsContent value="languages" className="mt-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {languages.map((skill, idx) => <SkillCard key={idx} skill={skill} idx={idx} />)}
            </div>
          </TabsContent>

          <TabsContent value="web" className="mt-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[...webAndDb, ...tools].map((skill, idx) => <SkillCard key={idx} skill={skill} idx={idx} />)}
            </div>
          </TabsContent>

          <TabsContent value="professional" className="mt-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {designSkills.map((skill, idx) => <SkillCard key={idx} skill={skill} idx={idx} isPro />)}
            </div>
          </TabsContent>
        </Tabs>
      </div>
...
function SkillCard({ skill, idx, isPro = false }) {
  const getIcon = (name) => {
    const n = name.toLowerCase();
