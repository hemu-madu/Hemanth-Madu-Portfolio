...
function Navbar() {
  ...
  return <header
    className={cn(
      ...
      scrolled ? "bg-background/90 backdrop-blur-xl shadow-md border-b border-border/50 py-2" : "bg-transparent py-4"
    )}
  >
  ...
  >
                  {link.label}
                  {activeSection === link.href.substring(1) && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-blue-600 to-purple-600 rounded-full" />}
                </a>)}
            </nav>
          </div>
  ...
  >
    ...
    className={cn(
      ...
      activeSection === link.href.substring(1) ? "text-primary bg-secondary/50" : "text-muted-foreground"
    )}
  >
                  {link.label}
                </a>)}
